...
--update table명 set column명 = 변경할 값, column명 = 변경할 값, .... where 행 선택 조건


drop table emp03;
create table emp03 as select * from emp;

update emp03 set deptno = deptno + 30;

select * from emp03;

alter table emp03 rename column comm to reword;

update emp03 set hiredate = sysdate;
update emp03 set deptno = deptno + 30 where deptno = 10;

update emp03 set deptno = 40 where ename ='SCOTT';

update emp03 set deptno = (select deptno from emp03 where ename = 'SCOTT') where ename = 'SMITH';

--where 조건문이 없다면 모든 행을 지우기 때문에 조건을 잘 줘야함
delete from emp03 where ename = 'KING';
delete from emp03 where deptno = 40;
delete from emp03 where deptno = all(select deptno from emp03 where job = 'SALESMAN');
