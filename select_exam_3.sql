...
select to_date(sysdate,'yy-mm-dd') - to_date('22/1/1','yy-mm-dd') as 지난날 from dual;
...
select ename, job, sal, decode(job, 'ANALYST', sal + 200,'SALESMAN', sal + 180,'MANAGER', sal + 150,'CLERK', sal + 100) as newsal from emp;
...
select job, deptno as 부서번호, count(*) as 사원수, decode(deptno, 10, 'a', 20, 'b', 30, 'c', 40, 'd') as 부서명 from emp group by job, deptno order by deptno;
