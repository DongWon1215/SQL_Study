...
select ename, job, emp.deptno, dname from emp join dept on emp.deptno = dept.deptno where loc = 'NEW YORK';
...
select e2.* from emp e1 join emp e2 on e1.deptno = e2.deptno where e1.ename ='SCOTT' and e2.ename != 'SCOTT';

--41. SELF JOIN을 사용하여 WARD 사원보다 늦게 입사한 사원의 이름과 입사일을 출력하시오.
select e2.* from emp e1 join emp e2 on e1.deptno = e2.deptno where e1.ename = 'WARD' and e1.hiredate < e2.hiredate;

--42. SELF JOIN 을 사용하여 관리자보다 먼저 입사한 모든 사원의 이름 및 입사일을 관리자의 이름 및 입사일과 함께 출력하시오.
select e1.ename, e1.hiredate from emp e1 join emp e2 on e1.mgr = e2.empno where e1.hiredate < e2.hiredate;